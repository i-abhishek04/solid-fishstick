...
class MapsGenerator {
    ...
    public MapsGenerator(int row, int col) {
        map = new int[row][col];
...
class BBPlay extends JPanel implements KeyListener, ActionListener {
    ...
    private MapsGenerator map;

    public BBPlay() {
        map = new MapsGenerator(3, 7);
        addKeyListener(this);
        setFocusable(true);
    ...
    public void keyPressed(KeyEvent arg0) {
        if (arg0.getKeyCode() == KeyEvent.VK_RIGHT) { // if right arrow key
            // is pressed then paddle moves right
            if (playerX >= 600) {
                playerX = 600;
            } else {
                moveRight();
        ...
        if (arg0.getKeyCode() == KeyEvent.VK_LEFT) { // if left arrow key is
            // pressed then paddle moves left
            if (playerX < 10) {
                playerX = 10;
            } else {
                moveLeft();
        ...
        if (arg0.getKeyCode() == KeyEvent.VK_ENTER) { // if enter key is
            // pressed then game restarts
            if (!play) {
                play = true;
                ballposX = 120;
                ballposY = 350;
